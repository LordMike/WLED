...
#endif
#ifndef PXP_TIMING_EPOCH_BACKSTEP_MS
  #define PXP_TIMING_EPOCH_BACKSTEP_MS 1000
#endif
...
uint32_t pxpTimingLastSenderMs = 0;
bool pxpTimingLastSenderValid = false;
...
void pxpApplyTimeSync(uint32_t senderMs, uint32_t receiveMs, bool trackSenderTime = true)
{
  ...
  if (!pxpTimingSynced) {
    ...
    if (trackSenderTime) {
      pxpTimingLastSenderMs = senderMs;
      pxpTimingLastSenderValid = true;
    }
  ...
  if (trackSenderTime && pxpTimingLastSenderValid) {
    const int32_t senderDelta = pxpSenderTimeDiff(senderMs, pxpTimingLastSenderMs);
    if (senderDelta < -int32_t(PXP_TIMING_EPOCH_BACKSTEP_MS)) {
      DEBUG_PRINTF_P(PSTR("PXP timing epoch reset: sender=%lu oldLast=%lu delta=%ld receive=%lu oldSender=%lu oldLocal=%lu q=%u/%u\n"),
                     (unsigned long)senderMs, (unsigned long)pxpTimingLastSenderMs, (long)senderDelta,
                     (unsigned long)receiveMs, (unsigned long)pxpTimingSenderSyncMs, (unsigned long)pxpTimingLocalSyncMs,
...
void pxpResetTiming()
{
  ...
  pxpTimingLastSenderMs = 0;
  pxpTimingLastSenderValid = false;
    ...
    switch (commandType) {
      ...
      case PXP_CMD_SCHEDULE: {
        ...
        if (timeCode & 0x01) {
          ...
          if (!pxpTimingSynced) pxpApplyTimeSync(timeMs, receiveMs, false);
